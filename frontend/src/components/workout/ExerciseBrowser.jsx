...
function ProxyGif({ exerciseId, name, size = "100%" }) {
  ...
  return (
    <img
      src={`/api/v1/exercises/${exerciseId}/gif`}
      alt={name}
...
function ExerciseDetailModal({ exercise, onClose }) {
  ...
  useEffect(() => {
    ...
    exerciseAPI.get(exercise.id)
      ...
      .catch(() => setLoadingGuidance(false));
  }, [exercise?.id]);
    ...
    >
      ...
      >
        ...
        <div style={{ display: "flex", flexWrap: "wrap", gap: 8 }}>
          ...
          {exercise.difficulty && (
            <Badge
              color={exercise.difficulty === "beginner" ? T.teal : exercise.difficulty === "advanced" ? T.negative : T.amber}
              size="sm"
            >
              {exercise.difficulty}
            </Badge>
          ...
          {MUSCLE_GROUPS.map((m) => (
            <button
              ...
              style={{
                ...
                fontSize: 12,
                fontWeight: muscle === m.id ? 700 : 500,
                cursor: "pointer",
                fontFamily: "inherit",
                whiteSpace: "nowrap",
              }}
                ...
                try {
                  ...
                  if (result.items && result.items.length > 0) {
                    ...
                    setDetailExercise({
                      ...
                      id: dbEx.id,
                      name: dbEx.name,
                      primary: Array.isArray(dbEx.primary_muscles) && dbEx.primary_muscles.length > 0
                        ? dbEx.primary_muscles[0].toLowerCase()
                        : ex.primary,
                      secondary: Array.isArray(dbEx.secondary_muscles) && dbEx.secondary_muscles.length > 0
                        ? dbEx.secondary_muscles.map((m) => m.toLowerCase())
                        : ex.secondary,
